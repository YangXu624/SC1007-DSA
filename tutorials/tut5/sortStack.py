...
def sort_stack(stack):
    ...
    while not temp_stack.is_empty():
        curr = temp_stack.pop()
        count = 0 # keeps track of number of items held temporarily
        while not stack.is_empty() and stack.peek() < curr:
            temp_stack.push(stack.pop()) # pop all items smaller than curr
            count += 1
        stack.push(curr) # push curr
        while count > 0:
            stack.push(temp_stack.pop()) # bring the items back
            count -= 1
    return stack
